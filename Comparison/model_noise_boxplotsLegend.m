...
for actualNoiseMode=1:size(noiseMode,2)
    ...
    c = get(gca, 'Children');
    hleg1 = legend(c(1:size(algorithms,1)), algorithms{size(algorithms,1):-1:1},'Location','southeast','Orientation','vertical');
    hleg1.NumColumns = 2;
    hleg1.FontSize = 9;
    hleg1.FontName = 'Times New Roman';
    hleg1.FontWeight = 'normal';
    ...
    savefig(['plots\Sets\results_' noiseMode{actualNoiseMode} '_' Name 'BoxplotLegend.fig'])
    saveas(gcf,['plots\Sets\results_' noiseMode{actualNoiseMode} '_' Name 'BoxplotLegend.png'])
    matlab2tikz(['plots\Sets\results_' noiseMode{actualNoiseMode} '_' Name 'BoxplotLegend.tex'])
