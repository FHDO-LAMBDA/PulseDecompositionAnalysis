function [signal_mod,y,opt_val_sort] = decomposeGamma3sorelliMode(ppg_signal,freq,varargin)
% Pulse Decomposition Analysis: Gamma3 algorithm (Sorelli restrictions,
%                               mode definition)
%
% References: 
...
% their respective maxima location. The outputs are the 3 optimized basis 
% functions, the optimized parameters of the kernels as well as the 
% reconstructed signal which is the sum of the 3 basis functions.
% 
% input: 
...
if(~(isempty(noOpt)) && (noOpt==true)) % is noOpt is set true, skip optimization
    ...
    opt_values=fmincon(h,x0,[],[],[],[],lb,ub,@constr_sorelli3kernels,options);
