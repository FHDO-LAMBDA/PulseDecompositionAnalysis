function [signal_mod,y,opt_val_sort] = decomposeGamma4sorelliMode(ppg_signal,freq,varargin)
...
s4=@(x) ((1/(x(12)))*((beta4(x)^alpha4(x))/(gamma(alpha4(x)))*((alpha4(x)-1)/beta4(x))^(alpha4(x)-1)*exp(1-alpha4(x))));
gamma4=@(x) ((beta4(x)^alpha4(x))/(s4(x)*gamma(alpha4(x))))*t_ppg.^(alpha4(x)-1).*exp(-(beta4(x))*t_ppg);
% combining the waves
g=@(x) gamma1(x)+gamma2(x)+gamma3(x)+gamma4(x);
