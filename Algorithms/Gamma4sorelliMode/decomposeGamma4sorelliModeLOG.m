function [signal_mod,y,opt_val_sort] = decomposeGamma4sorelliModeLOG(ppg_signal,freq,varargin)
% Pulse Decomposition Analysis: Gamma4 algorithm (Sorelli restrictions,
%                               mode definition)
%
% References: 
% Timo Tigges u. a. „In-ear photoplethysmography for central pulse waveform
% analysis in non-invasive hemodynamic monitoring“. 
% In: Current Directions in Biomedical Engineering 3.2 (2017), S. 587–590.
%
...
%                 for derivation
%
% This function decomposes a single beat of a ppg signal by using 4 gamma
% functions (defined by their mode) as basis functions with the restrictions
% of Sorelli et al. The optimization algorithm sorts the basis functions by
% their respective maxima location. The outputs are the 4 optimized basis 
% functions, the optimized parameters of the kernels as well as the 
% reconstructed signal which is the sum of the 4 basis functions.
% 
% input: 
...
%
% 'y':              cell array of size 4x1 which contains the 4 basis
%                   functions (y1, y2, y3, y4) each with a size of 
...
if (isempty(x0))
    ...
    %% initial values
    % 1. wave
    x0(1)=delta_t_sys/(2*sqrt(2*log(2))); % width (sigma)
    x0(2)=t_ppg(1)+0.5*delta_t_sys; % position (my)
    x0(3)=0.8*max(ppg_signal); % amplitude (a)
    % 2. wave
    x0(4)=0.25*delta_t_dia/(2*sqrt(2*log(2))); % width (sigma)
    x0(5)=inc_ref; % position (my)
    x0(6)=0.4*max(ppg_signal); % amplitude (a)
    % 3. wave
    x0(7)=0.25*delta_t_dia/(2*sqrt(2*log(2))); % width (sigma)
    x0(8)=inc_ref+0.33*delta_t_dia; % position (my)
    x0(9)=0.4*max(ppg_signal); % amplitude (a)
    % 4. wave
    x0(10)=0.25*delta_t_dia/(2*sqrt(2*log(2))); % width (sigma)
    %x0(11)=inc_ref+0.67*delta_t_dia; % position (my)
    x0(11) = 0.5;
    x0(12)=0.4*max(ppg_signal); % amplitude (a)
end

%% boundaries
lb=[0.5*delta_t_sys/(2*sqrt(2*log(2))) t_ppg(1) 0.5*max(ppg_signal) ...
    0.1*delta_t_dia/(2*sqrt(2*log(2))) inc_ref 0 ...
    0.1*delta_t_dia/(2*sqrt(2*log(2))) inc_ref 0 ...
    0.1*delta_t_dia/(2*sqrt(2*log(2))) inc_ref 0]; % lower boundary
ub=[1.5*delta_t_sys/(2*sqrt(2*log(2))) inc_ref max(ppg_signal)...
    0.33*delta_t_dia/(2*sqrt(2*log(2))) max(t_ppg) 0.6*max(ppg_signal)...
    0.33*delta_t_dia/(2*sqrt(2*log(2))) max(t_ppg) 0.6*max(ppg_signal)...
    0.33*delta_t_dia/(2*sqrt(2*log(2))) max(t_ppg) 0.6*max(ppg_signal)]; % upper boundary

%% optimization
% 1. wave
alpha1=@(x) ((1/(2*(x(1)^2)))*(x(2)^2+x(2)*sqrt(x(2)^2+4*(x(1)^2)))+1);
beta1=@(x) ((1/(2*(x(1)^2)))*(x(2)+sqrt(x(2)^2+4*(x(1)^2))));
s1=@(x) ((1/(x(3)))*(beta1(x)^alpha1(x))/(gamma(alpha1(x)))*((alpha1(x)-1)/beta1(x))^(alpha1(x)-1)*exp(1-alpha1(x)));
gamma1=@(x) ((beta1(x)^alpha1(x))/(s1(x)*gamma(alpha1(x))))*t_ppg.^(alpha1(x)-1).*exp(-(beta1(x))*t_ppg);
% 2. wave
alpha2=@(x) ((1/(2*(x(4)^2)))*(x(5)^2+x(5)*sqrt(x(5)^2+4*(x(4)^2)))+1);
beta2=@(x) ((1/(2*(x(4)^2)))*(x(5)+sqrt(x(5)^2+4*(x(4)^2))));
s2=@(x) ((1/(x(6)))*(beta2(x)^alpha2(x))/(gamma(alpha2(x)))*((alpha2(x)-1)/beta2(x))^(alpha2(x)-1)*exp(1-alpha2(x)));
gamma2=@(x) ((beta2(x)^alpha2(x))/(s2(x)*gamma(alpha2(x))))*t_ppg.^(alpha2(x)-1).*exp(-(beta2(x))*t_ppg);
% 3. wave
alpha3=@(x) ((1/(2*(x(7)^2)))*(x(8)^2+x(8)*sqrt(x(8)^2+4*(x(7)^2)))+1);
beta3=@(x) ((1/(2*(x(7)^2)))*(x(8)+sqrt(x(8)^2+4*(x(7)^2))));
s3=@(x) ((1/(x(9)))*(beta3(x)^alpha3(x))/(gamma(alpha3(x)))*((alpha3(x)-1)/beta3(x))^(alpha3(x)-1)*exp(1-alpha3(x)));
gamma3=@(x) ((beta3(x)^alpha3(x))/(s3(x)*gamma(alpha3(x))))*t_ppg.^(alpha3(x)-1).*exp(-(beta3(x))*t_ppg);
% 4. wave
alpha4=@(x) ((1/(2*(x(10)^2)))*(x(11)^2+x(11)*sqrt(x(11)^2+4*(x(10)^2)))+1);
beta4=@(x) ((1/(2*(x(10)^2)))*(x(11)+sqrt(x(11)^2+4*(x(10)^2))));
lns4=@(x) log((1/(x(12)*gamma(alpha4(x))))) + ((beta4(x)*log(alpha4(x)))+((alpha4(x)-1)*log((alpha4(x)-1)/beta4(x)))+(1-alpha4(x)));
lngamma4=@(x) (alpha4(x)-1).*log(t_ppg)-log(gamma(alpha4(x)))-lns4(x)+(beta4(x)*log(alpha4(x)))-(beta4(x).*t_ppg);
% combining the waves
g=@(x) gamma1(x)+gamma2(x)+gamma3(x)+round(exp(lngamma4(x)),7);
...
if(~(isempty(noOpt)) && (noOpt==true)) % is noOpt is set true, skip optimization
    ...
    opt_val_sort = x0;
    % 1. wave
    y(1,:) = gamma1(opt_val_sort);
    % 2. wave
    y(2,:) = gamma2(opt_val_sort);
    % 3. wave
    y(3,:) = gamma3(opt_val_sort);
    % 4. wave
    y(4,:) = exp(lngamma4(opt_val_sort));
    % sum of basis functions
    signal_mod=sum(y); % reconstructed signal
    % produce cell array of basis functions
    y = mat2cell(y,[1 1 1 1],length(ppg_signal));
    
else % do optimization
    
    options = optimoptions(@fmincon,'MaxFunctionEvaluations',Inf);
    opt_values=fmincon(h,x0,[],[],[],[],lb,ub,@constr_sorelli4kernels,options);
    
    %% reconstruction
    % 1. wave
    yTmp(1,:) = gamma1(opt_values);
    % 2. wave
    yTmp(2,:) = gamma2(opt_values);
    % 3. wave
    yTmp(3,:) = gamma3(opt_values);
    % 4. wave
    yTmp(4,:) = exp(lngamma4(opt_values));
