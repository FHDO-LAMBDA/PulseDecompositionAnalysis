function [signal_mod,y,opt_val_sort] = decomposeGauss5(ppg_signal,freq,varargin)
% Pulse Decomposition Analysis: Gauss5 algorithm (free positioning)
%
% References:
% -
%
% This function decomposes a single beat of a ppg signal by using 5
...
% respective maxima location. The outputs are the 5 optimized basis 
% functions, the optimized parameters of the kernels as well as the 
% reconstructed signal which is the sum of the 5 basis functions.
% 
% input: 
...
% specify constraints
function [c, ceq] = constr_free5kernels(x)
...
if(~(isempty(noOpt)) && (noOpt==true)) % is noOpt is set true, skip optimization
    ...
    opt_values = fmincon(h,x0,[],[],[],[],lb,ub,@constr_free5kernels,options);
