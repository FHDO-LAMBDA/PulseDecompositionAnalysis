function [signal_mod,y,opt_val_sort] = decomposeGamma3(ppg_signal,freq,varargin)
% Pulse Decomposition Analysis: Gamma3 algorithm (free positioning, mean
%                               definition)
%
% References: 
...
beta2=@(x) (x(5)/(x(6)^2));
s2=@(x) ((1/(x(1)))*(beta2(x)^alpha2(x))/(gamma(alpha2(x)))*((alpha2(x)-1)/beta2(x))^(alpha2(x)-1)*exp(1-alpha2(x)));
