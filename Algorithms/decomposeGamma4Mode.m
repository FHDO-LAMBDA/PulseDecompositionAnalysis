function [signal_mod,y,opt_val_sort] = decomposeGamma4Mode(ppg_signal,freq,varargin)
% Pulse Decomposition Analysis: Gamma4 algorithm (free positioning, mode
%                               definition)
%
% References: 
...
% functions (defined by their mode) as basis functions with empirically 
% determined initial values. The optimization algorithm sorts the basis 
% functions by their respective maxima location. The outputs are the 4 
% optimized basis functions, the optimized parameters of the kernels as 
% well as the reconstructed signal which is the sum of the 4 basis 
% functions.
% 
% input: 
...
%% optimization
% 1. wave
alpha1=@(x) ((1/(2*(x(3)^2)))*(x(2)^2+x(2)*sqrt(x(2)^2+4*(x(3)^2)))+1);
beta1=@(x) ((1/(2*(x(3)^2)))*(x(2)+sqrt(x(2)^2+4*(x(3)^2))));
s1=@(x) ((1/(x(1)))*(beta1(x)^alpha1(x))/(gamma(alpha1(x)))*((alpha1(x)-1)/beta1(x))^(alpha1(x)-1)*exp(1-alpha1(x)));
gamma1=@(x) ((beta1(x)^alpha1(x))/(s1(x)*gamma(alpha1(x))))*t_ppg.^(alpha1(x)-1).*exp(-(beta1(x))*t_ppg);
% 2. wave
alpha2=@(x) ((1/(2*(x(6)^2)))*(x(5)^2+x(5)*sqrt(x(5)^2+4*(x(6)^2)))+1);
beta2=@(x) ((1/(2*(x(6)^2)))*(x(5)+sqrt(x(5)^2+4*(x(6)^2))));
s2=@(x) ((1/(x(4)))*(beta2(x)^alpha2(x))/(gamma(alpha2(x)))*((alpha2(x)-1)/beta2(x))^(alpha2(x)-1)*exp(1-alpha2(x)));
gamma2=@(x) ((beta2(x)^alpha2(x))/(s2(x)*gamma(alpha2(x))))*t_ppg.^(alpha2(x)-1).*exp(-(beta2(x))*t_ppg);
% 3. wave
alpha3=@(x) ((1/(2*(x(9)^2)))*(x(8)^2+x(8)*sqrt(x(8)^2+4*(x(9)^2)))+1);
beta3=@(x) ((1/(2*(x(9)^2)))*(x(8)+sqrt(x(8)^2+4*(x(9)^2))));
s3=@(x) ((1/(x(7)))*(beta3(x)^alpha3(x))/(gamma(alpha3(x)))*((alpha3(x)-1)/beta3(x))^(alpha3(x)-1)*exp(1-alpha3(x)));
gamma3=@(x) ((beta3(x)^alpha3(x))/(s3(x)*gamma(alpha3(x))))*t_ppg.^(alpha3(x)-1).*exp(-(beta3(x))*t_ppg);
% 4. wave
alpha4=@(x) ((1/(2*(x(12)^2)))*(x(11)^2+x(11)*sqrt(x(11)^2+4*(x(12)^2)))+1);
beta4=@(x) ((1/(2*(x(12)^2)))*(x(11)+sqrt(x(11)^2+4*(x(12)^2))));
s4=@(x) ((1/(x(10)))*(beta4(x)^alpha4(x))/(gamma(alpha4(x)))*((alpha4(x)-1)/beta4(x))^(alpha4(x)-1)*exp(1-alpha4(x)));
...
% specify constraints
function [c, ceq] = constr_free4kernels(x)
...
if(~(isempty(noOpt)) && (noOpt==true)) % is noOpt is set true, skip optimization
    ...
    opt_values=fmincon(h,x0,[],[],[],[],lb,ub,@constr_free4kernels,options);
