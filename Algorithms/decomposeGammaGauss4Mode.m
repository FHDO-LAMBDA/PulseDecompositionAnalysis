function [signal_mod,y,opt_val_sort] = decomposeGammaGauss4Mode(ppg_signal,freq,varargin)
% Pulse Decomposition Analysis: GammaGauss4 algorithm (free positioning, 
%                               mode definition)
%
% References: 
...
%
% Sheng-Cheng Huang u.a. „Evaluation of Decomposition Analysis on 
% Multi-Models for Digital Volume Pulse Signal“. In: IFMBE Proceedings 51.1 
% (2015), S. 1731 – 1734.
%
% This function decomposes a single beat of a ppg signal by using 1 gamma
% function (defined by its mode) and 3 gaussian functions as basis functions
% with empirically determined initial values. The optimization algorithm 
% sorts the basis functions by their respective maxima location. The 
% outputs are the 4 optimized basis functions, the optimized parameters of 
% the kernels as well as the reconstructed signal which is the sum of the 4
% basis functions.
% 
% input: 
...
% 2. wave
gauss1=@(x) (x(4)*exp(-(t_ppg-x(5)).^2/(2*x(6)^2)));
% 3. wave
gauss2=@(x) (x(7)*exp(-(t_ppg-x(8)).^2/(2*x(9)^2)));
% 4. wave
gauss3=@(x) (x(10)*exp(-(t_ppg-x(11)).^2/(2*x(12)^2)));
% combining the waves
g=@(x) gamma1(x)+gauss1(x)+gauss2(x)+gauss3(x);
...
if(~(isempty(noOpt)) && (noOpt==true)) % is noOpt is set true, skip optimization
    ...
    % 2. wave
    y(2,:) = gauss1(opt_val_sort);
    % 3. wave
    y(3,:) = gauss2(opt_val_sort);
    % 4. wave
    y(4,:) = gauss3(opt_val_sort);
    ...
    % 2. wave
    yTmp(2,:) = gauss1(opt_values);
    % 3. wave
    yTmp(3,:) = gauss2(opt_values);
    % 4. wave
    yTmp(4,:) = gauss3(opt_values);
