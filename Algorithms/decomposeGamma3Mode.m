function [signal_mod,y,opt_val_sort] = decomposeGamma3Mode(ppg_signal,freq,varargin)
% Pulse Decomposition Analysis: Gamma3 algorithm (free positioning, mode
%                               definition)
%
% References: 
...
%
% This function decomposes a single beat of a ppg signal by using 3 gamma
...
% functions by their respective maxima location. The outputs are the 3 
% optimized basis functions, the optimized parameters of the kernels as 
% well as the reconstructed signal which is the sum of the 3 basis 
% functions.
% 
% input: 
...
%
% 'y':              cell array of size 3x1 which contains the 3 basis
%                   functions (y1, y2, y3) each with a size of 
...
% combining the waves
g=@(x) gamma1(x)+gamma2(x)+gamma3(x);
h=@(x) sum((ppg_signal-g(x)).^2); % Residual Sum of Squares (RSS)

% specify constraints
function [c, ceq] = constr_free3kernels(x)
...
if(~(isempty(noOpt)) && (noOpt==true)) % is noOpt is set true, skip optimization
    ...
    y = mat2cell(y,[1 1 1],length(ppg_signal));
    
else % do optimization
    
    options = optimoptions(@fmincon,'MaxFunctionEvaluations',Inf);
    opt_values=fmincon(h,x0,[],[],[],[],lb,ub,@constr_free3kernels,options);
