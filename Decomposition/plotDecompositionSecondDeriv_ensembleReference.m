...
for actualPatientNumber=1:size(patients,1)
    ...
    for actualAlgorithm = 1:size(algorithms,1)
        ...
        for currentInterval=1:6
            ...
            title({[fileID ' ' epochs{currentInterval}]; algorithmName},'Interpreter','none')
            
            
            %% show decomposition
            myaxSignal(currentInterval+6)=subplot(4,3,currentInterval+6);%myaxSignal(currentInterval+6)
            if(~isempty(y{1}))
                plot(beatStartIndexReference:beatStopIndexReference,physiologicalMeasuresTable.([epochs{currentInterval} '_meanSeg']){actualPatientNumber,1}(beatStartIndexReference:beatStopIndexReference),'LineWidth',3); hold on
                plot(beatStartIndexReference:beatStopIndexReference,signal_mod,'Color','r')
                for i = 1:size(y,1)
                    plot(beatStartIndexReference:beatStopIndexReference,y{i},'k')
                end
                hold off
            else
                plot(1:numel(physiologicalMeasuresTable.([epochs{currentInterval} '_meanSeg']){actualPatientNumber,1}),0)
            end
            title({[fileID ' ' epochs{currentInterval}]; algorithmName},'Interpreter','none')
